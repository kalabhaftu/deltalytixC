 ...
 * Migrates all base64 images from database to Supabase storage using Prisma
 * 
 * Usage: tsx scripts/migrate-images-prisma.ts
 */
...
/**
 * Step 1: Fetch all trades with base64 images using Prisma
 */
async function fetchTradesWithBase64() {
  ...
  const trades = await prisma.trade.findMany({
    where: {
      OR: [
        ...
        { imageBase64Second: { startsWith: 'data:image/' } },
        { imageBase64Third: { startsWith: 'data:image/' } },
        { imageBase64Fourth: { startsWith: 'data:image/' } },
        { imageBase64Fifth: { startsWith: 'data:image/' } },
        { imageBase64Sixth: { startsWith: 'data:image/' } },
        { cardPreviewImage: { startsWith: 'data:image/' } },
      ]
    },
    select: {
      id: true,
      ...
      imageBase64Third: true,
      imageBase64Fourth: true,
      imageBase64Fifth: true,
      imageBase64Sixth: true,
      cardPreviewImage: true,
    }
  })
  
  console.log(`✅ Found ${trades.length} trades with base64 images`)
  return trades
}
 ...
 */
async function createBackupZip(trades: any[]): Promise<string> {
  console.log('\n💾 Creating backup ZIP...')
  ...
  return new Promise((resolve, reject) => {
    ...
    for (const trade of trades) {
      for (const field of IMAGE_FIELDS) {
        const base64Data = (trade as any)[field]
...
async function migrateAllImages(
  ...
  for (const trade of trades) {
    for (const field of IMAGE_FIELDS) {
      if ((trade as any)[field] && (trade as any)[field].startsWith('data:image/')) {
        totalImages++
      }
    }
  }
    ...
    for (const field of IMAGE_FIELDS) {
      ...
      if (base64Data && base64Data.startsWith('data:image/')) {
        try {
          ...
          // Update database with storage URL using Prisma
          await prisma.trade.update({
            where: { id: trade.id },
            data: { [field]: storageUrl }
          })
