...
interface WinRateKpiProps {
  size?: WidgetSize
}

// Custom Gauge Component for Trade Win Rate
function TradeWinGaugeChart({ value }: { value: number }) {
  ...
  // Color based on percentage - green for good (≥50%), red for poor (<50%)
  const getColor = (val: number) => {
    if (val >= 50) return '#10b981' // green-500 for good performance
    return '#ef4444' // red-500 for poor performance
  }
...
export default function WinRateKpi({ size = 'kpi' }: WinRateKpiProps) {
  const { winRate } = useTradeStatistics()

  // Determine if win rate is good
      ...
      <CardContent className="flex flex-col justify-between h-full p-3">
            ...
            <TooltipProvider delayDuration={100}>
              <Tooltip>
                ...
                <TooltipContent side="bottom" sideOffset={5} className="max-w-[300px]">
                  Percentage of winning trades compared to losing trades (excluding break-even trades).
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          </div>
        ...
        <div className="flex-1 flex items-center justify-between gap-4">
          ...
          <div className="relative">
            <TradeWinGaugeChart value={winRate} />
          </div>
        </div>
